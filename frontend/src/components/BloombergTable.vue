  ...
  >
    <div class="w-full max-w-5xl mx-auto">
      <div class="text-center mb-2">
        <h1 class="text-3xl font-extrabold text-white mt-5 mb-1 tracking-wide">StocksApp</h1>
        <span class="text-gray-400 text-base block mb-2">by Juan Felipe Garcia</span>
      </div>
          ...
          <tbody class="bg-black">
            ...
            >
              <td :class="[
                ...
                hovered === row.ticker ? 'bg-gradient-to-r from-yellow-700 to-yellow-900' : '',
                'group-hover:bg-gradient-to-r group-hover:from-yellow-700 group-hover:to-yellow-900'
              ]">{{ row.ticker }}</td>
              <td class="px-3 py-2 h-8 min-h-[32px] align-middle">{{ row.valor }}</td>
              <td :class="[row.valcambio > 0 ? 'text-green-400' : row.valcambio < 0 ? 'text-red-400' : 'text-white', 'px-3 py-2 h-8 min-h-[32px] align-middle']">{{ formatChange(row.valcambio) }}</td>
              <td :class="[row.porcencambio > 0 ? 'text-green-400' : row.porcencambio < 0 ? 'text-red-400' : 'text-white', 'px-3 py-2 h-8 min-h-[32px] align-middle']">{{ formatChange(row.porcencambio, true) }}</td>
              <td class="text-gray-400 text-sm px-3 py-2 h-8 min-h-[32px] align-middle">{{ row.hora }}</td>
            </tr>
          </tbody>
        </table>
      </div>
